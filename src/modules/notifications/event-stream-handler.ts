...
// SSE 事件流核心（Web /notifications/stream 与小程序 /api/miniapp/events/stream 共用）：
// LISTEN 唤醒 + EventRecord 游标回放 + 25s 心跳 + transient 正在输入事件。
...
function parseTransientEvent(value: string | null) {
  ...
  try {
    ...
    return {
      type: envelope.type,
      userIds: envelope.userIds as string[],
